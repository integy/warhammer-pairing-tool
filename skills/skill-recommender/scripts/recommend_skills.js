...
function resolveSkillsDir(rawDir) {
  const baseDir = path.resolve(process.cwd());
  const requested = rawDir || 'skills';
  if (typeof requested !== 'string' || requested.trim() === '') {
    throw new Error('skills_dir must be a non-empty relative path');
  }
  if (path.isAbsolute(requested)) {
    throw new Error('skills_dir must be relative to the current workspace');
  }
  const resolved = path.resolve(baseDir, requested);
  const relative = path.relative(baseDir, resolved);
  if (relative === '' || relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error('skills_dir must stay inside the current workspace');
  }
  return resolved;
}
...
function listSkills(skillsDir) {
  const resolvedDir = resolveSkillsDir(skillsDir);
...
function recommendSkills(input) {
  ...
  const skills = listSkills(input.skills_dir);
  const ranked = skills
    .map(skill => {
...
module.exports = { recommendSkills, listSkills, readSkillMeta, resolveSkillsDir };
